...
class AKT_qid(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if self.n_pid > 0:
            ...
            self.pro_embed_diff = nn.Embedding(self.n_pid + 1, embed_l)
    ...
    def base_emb(self, q_data, target):
        q_embed_data = self.pro_embed(q_data)  # BS, seqlen,  d_model# c_ct
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        if self.n_pid > 0: # have problem id
            ...
            q_embed_diff_data = self.pro_embed_diff(pid_data)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'akt', "akt_enhance_pro", "akt_enhance_pro_qid", "akt_qid"}:
