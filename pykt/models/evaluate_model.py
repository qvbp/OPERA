...
def evaluate(model, test_loader, model_name, rel=None, save_path=""):
    ...
    with torch.no_grad():
        ...
        # 为OT任务添加评估指标收集器
        if model_name in ["dkt_otkt", "dkt_kt_ot"]:
            ot_correct = 0
            ot_total = 0
        ...
        for data in test_loader:
            ...
            if model.model_name in que_type_models and model_name not in ["lpkt", "rkt", "lightkt", "expertkt", "pointkt", "dkt_enhance_pro", "dkt_kt_ot", "sakt_pro", "sakt_enhance_pro", "dkvmn_pro", "dkvmn_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid", "akt_enhance_pro_qid", "akt_qid"]:
                model.model.eval()
            else:
                model.eval()
            ...
            if model_name in ["atdkt"]:
                ...
                # 获取学生选择的选项
                if "student_opts" in dcur and "shft_student_opts" in dcur:
                    s_opts = dcur["student_opts"].to(device)
                    s_opts_shft = dcur["shft_student_opts"].to(device)
                else:
                    print("Warning: student_opts not found in evaluation data!")
                    s_opts = torch.zeros_like(c).to(device)
                    s_opts_shft = torch.zeros_like(cshft).to(device)
                ...
                # 计算OT任务准确率
                pred_opts = torch.argmax(ot, dim=-1)  # 获取预测的选项索引
                
                # 只在有效位置(sm为True)计算准确率
                valid_indices = sm.nonzero(as_tuple=True)
                pred_valid = pred_opts[valid_indices]
                target_valid = s_opts_shft[valid_indices]
                
                # 累加正确预测的数量
                ot_correct += (pred_valid == target_valid).sum().item()
                ot_total += len(valid_indices[0])

            elif model_name in ["lightkt", "pointkt"]:   
                ...
                if "student_opts" in dcur and "shft_student_opts" in dcur:
                    s_opts = dcur["student_opts"].to(device)
                    s_opts_shft = dcur["shft_student_opts"].to(device)
                    c_opts = dcur["correct_opts"].to(device) if "correct_opts" in dcur else None
                    
                    # 获取KT和OT任务的预测 - 使用命名参数确保正确性
                    ...
                    # KT任务使用question level评估
                    y = kt_pred
                    c, cshft = q, qshft  # question level，和dkt_enhance_pro对齐
                    ...
                    if ot_pred is not None:
                        ...
                        if len(valid_indices[0]) > 0:
                            ...
                            # 获取有效位置的预测logits和目标选项
                            valid_ot_logits = ot_pred[batch_idx, seq_idx, :]  # [num_valid, max_options]
                            target_valid = s_opts_shft[batch_idx, seq_idx]    # [num_valid]
                            
                            # 计算每个位置的选项数（用于mask无效选项）
                            if c_opts is not None:
                                c_opts_shft = dcur["shft_correct_opts"].to(device)
                                s_opts_valid = s_opts_shft[batch_idx, seq_idx]
                                c_opts_valid = c_opts_shft[batch_idx, seq_idx] 
                                
                                # 计算每个位置的实际选项数
                                option_counts = torch.max(s_opts_valid, c_opts_valid) + 1
                                option_counts = torch.clamp(option_counts, min=2, max=model.max_options)
                                
                                # 创建选项mask
                                option_indices = torch.arange(model.max_options, device=device)
                                option_mask = option_indices.unsqueeze(0) < option_counts.unsqueeze(1)  # [num_valid, max_options]
                                
                                # 对无效选项位置设置极小值，确保不会被选中
                                masked_logits = valid_ot_logits.masked_fill(~option_mask, -1e9)
                                pred_opts = torch.argmax(masked_logits, dim=-1)
                            else:
                                # 如果没有correct_opts信息，直接使用argmax
                                pred_opts = torch.argmax(valid_ot_logits, dim=-1)
                            
                            # 只对有效的学生选择计算准确率
                            valid_target_mask = (target_valid >= 0) & (target_valid < model.max_options)
                            
                            if valid_target_mask.any():
                                valid_pred_opts = pred_opts[valid_target_mask]
                                valid_target_opts = target_valid[valid_target_mask]
                                
                                # 累加正确预测的数量
                                ot_correct += (valid_pred_opts == valid_target_opts).sum().item()
                                ot_total += valid_target_mask.sum().item()
                            
                else:
                    # 如果没有选项数据，只进行KT任务评估
                    print("Warning: student_opts not found in evaluation data for dkt_kt_ot!")
                    kt_pred, _ = model(
                        last_pro=q.long(), 
                        last_ans=r.long(), 
                        last_skill=c.long(), 
        ...
        # 如果是选项追踪模型
        if model_name in ["dkt_otkt", "dkt_kt_ot"]:
            ot_acc = ot_correct / ot_total if ot_total > 0 else 0
            print(f"OT Accuracy: {ot_acc:.4f}")
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "cakt", "folibikt","dtransformer","simplekt", "fliicbsimplekt", "simplekt_enhance_pro", "bakt_time", "sparsekt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "akt_enhance_pro"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt", "cakt", "folibikt", "dtransformer", "simplekt", "fliicbsimplekt", "simplekt_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid", "bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt", "akt_enhance_pro"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt", "cakt", "folibikt","simplekt", "fliicbsimplekt","simplekt_enhance_pro", "bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt", "akt_enhance_pro"]
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn","skvmn"]:
            ...
            elif model_name in ["simplekt", "fliicbsimplekt", "simplekt_enhance_pro", "sparsekt"]:
                y, h = model(dcurori, qtest=True, train=False)
                y = y[:,1:]
            elif model_name in ["akt", "cakt", "folibikt","akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "akt_enhance_pro"]:
                ...
                y, _ = model(q.long(), c.long(), r.long(), s_opts.long())
                y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
            elif model_name in ["dkt_forget"]:
                y = model(c.long(), r.long(), dgaps)
                y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["dkt_otkt"]:
            y, _ = model(qin.long(), cin.long(), rin.long(), rin.long())
            pred = y[0][-1][cout.item()]
        if model_name in ["dkt_forget", "bakt_time"]:
            din = dict()
...
def predict_each_group2(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    while bidx < finalcs.shape[0]:
        ...
        if model_name in ["atdkt"]:
            ...
            y, _ = model(curq.long(), curc.long(), curr.long(), curr.long())
            y = (y * one_hot(curcshft.long(), model.num_c)).sum(-1)
        elif model_name in ["dkt_forget"]:
            ...
            y = y[:, 1:]
        elif model_name in ["akt", "cakt", "folibikt", "cakt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "akt_enhance_pro"]:                                
