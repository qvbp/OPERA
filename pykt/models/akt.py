...
class AKT(nn.Module):
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        if not qtest:
            if self.cnt < 20:
                # print(f"c_reg_loss: {c_reg_loss}")
                self.cnt = self.cnt + 1
            return preds, c_reg_loss
        else:
            if self.cnt < 20:
                # print(f"xxxxx c_reg_loss xxxxx: {c_reg_loss}")
                self.cnt = self.cnt + 1
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
        ...
        self.ICB = ICB(d_model, 512, dropout)
        # self.cnt = 0
    ...
    def forward(self, mask, query, key, values, apply_pos=True, pdiff=None):
        ...
        if apply_pos:
            ...
            # if self.cnt < 10:
            #     print(f'**************query2.shape {query2.shape}****************')
            # self.cnt = self.cnt + 1
...
class ICB(nn.Module):
    def __init__(self, d_model, dim_feedforward, dropout=0.1):
        ...
        self.conv1 = nn.Conv1d(d_model, dim_feedforward, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(dim_feedforward, d_model, kernel_size=3, padding=1)
        self.activation = nn.ReLU()
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        x = x.transpose(1, 2)  # 转置为 [batch_size, d_model, seq_len]
        x = self.conv1(x)
        x = self.activation(x)
        x = self.dropout(x)
        x = self.conv2(x)
        x = x.transpose(1, 2)  # 再次转置回 [batch_size, seq_len, d_model]
        return x
