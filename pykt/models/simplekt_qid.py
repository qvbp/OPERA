...
class simpleKT_qid(nn.Module):
    def __init__(self, n_question, n_pid, 
        ...
        if self.n_pid > 0:
            ...
            self.q_embed_diff = nn.Embedding(self.n_pid+1, embed_l) # question emb, 总结了包含当前question（concept）的problems（questions）的变化
            self.qa_embed_diff = nn.Embedding(2 * self.n_question + 1, embed_l) # interaction emb, 同上
        
        if emb_type.startswith("qid"):
            # n_question+1 ,d_model
            self.q_embed = nn.Embedding(self.n_question, embed_l)
            self.pro_embed = nn.Embedding(self.n_pid+1, embed_l)
            if self.separate_qa: 
    ...
    def base_emb(self, q_data, target):
        ...
        q_embed_data = self.pro_embed(q_data).to(device)
        if self.separate_qa:
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if self.n_pid > 0 and emb_type.find("norasch") == -1: # have problem id
            if emb_type.find("aktrasch") == -1:  # 启动当前分支
                ...
                q_embed_diff_data = self.q_embed_diff(pid_data).to(device)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'simplekt', "simplekt_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid"}:
