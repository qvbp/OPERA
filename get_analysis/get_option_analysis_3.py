...
from typing import Dict, List, Any, Tuple  # 添加Tuple导入
...
class TextOnlyOptionAnalyzer:
    ...
    def create_analysis_prompt(self, question_data: Dict) -> Tuple[str, Dict]:
        ...
        for option_letter in option_letters:
            ...
            option_template = {
                "involved_kcs": ["Select specific knowledge concepts from the question's knowledge points"],
                "mastery_level": {
                    "specific_knowledge_concept_name": "mastery_level(0-4_integer, determined_dynamically_based_on_involved_knowledge_points)"
                },
                "reasoning": "Analysis of student cognition for choosing this option",
                    ...
                    "error_types": [
                        {
                            ...
                            "description": "specific_error_description", 
                            "suggestion": "specific_improvement_suggestions_for_this_error_type"
                        }
                    ],
                    "formatted_analysis": "Error_Type:[specific_error_type_name];Error_Description:[specific_error_description];Suggestion:[specific_improvement_suggestions_for_this_error_type]"
                }
        ...
        prompt = f"""
    You are an educational psychology and subject teaching expert. Please analyze the following English math multiple-choice question, focusing on analyzing student error types.

    【Important Notes】
    - The question and options may contain LaTeX mathematical formulas, please understand and analyze them correctly
    - Analyze the question requirements based on knowledge points: "{', '.join(available_kcs)}"
    - **The correct answer has been provided to you, please analyze based on this correct answer**
    - Focus on analyzing the error reasons and types of incorrect options

    【Question Information】
    Question ID: {question_data['jy_question_id']}
    Related Knowledge Points: {', '.join(available_kcs)}
    **Correct Answer: {correct_answer}**
    **Number of Options: {options_num}**

    【Complete Question (including all options)】
    {clean_problem_text}

    【Analysis Tasks】
    1. **Based on the known correct answer ({correct_answer})**: Please explain why this option is correct
    2. **Error Type Analysis**: Conduct detailed error type analysis for each incorrect option, analyzing why students would choose incorrect options

    【Error Type Analysis Requirements】
    Refer to the following error analysis framework:

    ## Math Multiple Choice Question Error Analysis Task
    Your task is to analyze student error types in math multiple choice questions.

    ### Analysis Steps
    1. Carefully read the question content and understand the requirements
    2. **Analyze the correct problem-solving process based on the provided correct answer ({correct_answer})**
    3. Compare the differences between other options and the correct answer
    4. Analyze the specific reasons why students would choose incorrect options
    5. Classify error reasons into corresponding error types
    6. Provide specific improvement suggestions for each error type

    ### Common Error Types Reference
    - Calculation Error: Basic arithmetic mistakes
    - Comprehension Error: Misunderstanding of question meaning
    - Concept Confusion: Unclear distinction of related concepts
    - Method Error: Inappropriate choice of problem-solving methods
    - Step Omission: Incomplete problem-solving process
    - Unit Error: Unit conversion or notation errors
    - Copying Error: Copying mistakes in questions or calculation process
    - Reading Error: Missing or misreading question conditions
    - Format Error: Incorrect answer format or problem-solving step writing

    【Analysis Requirements for Each Option】
    Analyze the following content for each option:
    - **Involved Knowledge Points**: Select from the following knowledge points: ["{kcs_str}"]
    - **Knowledge Mastery Level Assessment**: If students choose this option, reflect their mastery level of related knowledge points (evaluate each involved knowledge point)
    - **Choice Reason Analysis**: Why students would choose this option (correct reasoning/incorrect understanding/guessing, etc.)
    - **Error Type Analysis** (only for incorrect options): Analyze error types and suggestions according to specified format

    **Mastery Level Scoring Standard (0-4 levels):**
    - Level 4 (Complete Mastery): Choose correct option with sufficient reasoning, complete understanding of concepts and applications
    - Level 3 (Good Mastery): Basic correct understanding but may have detail omissions, or correct option but insufficient reasoning
    - Level 2 (Partial Mastery): Some understanding but obvious misunderstandings, leading to wrong choices
    - Level 1 (Initial Understanding): Vague concept understanding but serious understanding deviations
    - Level 0 (Complete Lack of Mastery): Complete lack of understanding of related concepts, random selection or based on incorrect understanding

    **Note**: Each option may involve 1 or multiple knowledge points, please list all relevant knowledge points and their mastery levels in mastery_level according to actual situations
    ...
    {{
        ...
        "correct_answer_explanation": "Detailed problem-solving process and principle explanation based on correct answer {correct_answer}",
        "analysis": {options_json_template},
        "overall_analysis": "Overall question knowledge point examination analysis and difficulty assessment"
    }}

    Important Notes:
    1. Please ensure JSON format is correct and can be parsed by Python json.loads()
    2. correct_answer field must output: {correct_answer}
    3. involved_kcs must be selected from the given knowledge point list: ["{kcs_str}"]
    4. Mastery level scoring should be reasonable, reflecting the correspondence between option selection and knowledge mastery
    5. Error analysis only for incorrect options, format requirement: Error_Type:[type_name];Error_Description:[specific_description];Suggestion:[improvement_suggestions]
    6. Must include jy_question_id field for result correspondence
    7. formatted_analysis field in error_analysis must strictly follow the specified format
    8. Analyze based on the provided correct answer ({correct_answer}), no need to re-derive the correct answer
    9. The question has a total of {options_num} options, please analyze each option
    10. Values in mastery_level must be integers between 0-4, not strings
    11. Each option should list all knowledge points actually involved in mastery_level (may be 1, 2, or multiple), and knowledge point names must be consistent with those in involved_kcs
    12. All analysis content should be in English to match the English question content
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        results = analyzer.batch_analyze_multithreaded(
            data_file="../get_analysis/3/result/failed.json",
            output_dir="../get_analysis/3/results_individual_v1",
        ...
        failed_data = analyzer.export_failed_questions(
            results, "../get_analysis/3/result/failed_questions_0723_v1.json"
        )
