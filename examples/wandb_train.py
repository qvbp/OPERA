...
def main(params):
    ...
    with open("../configs/kt_config.json") as f:
        ...
        if model_name in ["dkvmn","deep_irt", "sakt", "saint","saint++", "akt", "cakt", "folibikt", "atkt", "lpkt", "skvmn", "dimkt", "akt_enhance_pro", "sakt_pro", "sakt_enhance_pro", "dkvmn_enhance_pro", "dkvmn_pro", "akt_enhance_pro_qid", "akt_qid"]:
            train_config["batch_size"] = 64 ## because of OOM
        if model_name in ["simplekt", "bakt_time", "sparsekt", "simplekt_abqr", "simplekt_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid"]:
            train_config["batch_size"] = 64 ## because of OOM
    ...
    if model_name in ["saint","saint++", "sakt", "atdkt", "simplekt", "bakt_time","folibikt", "simplekt_abqr", "simplekt_enhance_pro", "sakt_pro", "sakt_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid"]:
    ...
    if model_name in ["dkt_abqr", "dkt_abqr_kc", "lightkt", "expertkt", "pointkt", "dkt_otkt", "dkt", "dkt_enhance_pro", "iekt", "qikt", "dkt_kt_ot", "sakt_enhance_pro"]:
        emb_size = params['emb_size']
        dataset_name = params['dataset_name']
    if model_name in ["simplekt", "simplekt_abqr", "akt", "simplekt_enhance_pro", "akt_enhance_pro", "simplekt_enhance_pro_qid", "simplekt_qid", "akt_qid", "akt_enhance_pro_qid"]:
        emb_size = params['d_model']
        dataset_name = params['dataset_name']
    if model_name in ["dkvmn_enhance_pro"]:
        dataset_name = params['dataset_name']

    if model_name in ['simplekt_abqr']:  # 对抗学习的超参数获取
    ...
    if model_name == "rkt":
        ...
        testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch = \
            train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model, None, None, None, None, None, None,data_config[dataset_name], fold)
    else:
        ...
        # 判断是否是simplekt_abqr模型，如果是则传入对抗学习的超参数
        if model_name in ['simplekt_abqr']:
